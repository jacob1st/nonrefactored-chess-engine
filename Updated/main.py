...
def depth_test(game_board, depth, color):
    if depth == 0:
        return 1
    num_pos = 0
    i = 0
    while i <= 63:
        if game_board.current_board[i] and game_board.current_board[i] & 24 == color:
            ...
            for move in moves:
                ...
                # game_board = draw_screen_update(display, game_board)
                if color == 8:
                    num_pos += depth_test(game_board, depth - 1, 16)
                else:
                    num_pos += depth_test(game_board, depth - 1, 8)
                game_board.current_board[i] = piece
                game_board.current_board[move] = taken_piece
                # game_board = draw_screen_update(display, game_board)
        i += 1
    ...
    while run:
        ...
        for event in [pygame.event.wait()]+pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                print("Depth 1:", depth_test(game_board, 1, turn))
                print("Depth 2:", depth_test(game_board, 2, turn))
                print("Depth 3:", depth_test(game_board, 3, turn))
